...
<body>
	<div class="container">
		<div class="row">
			<a href="<c:url value='/addPersonNew' />" class="btn btn-primary">Add
				Person</a>
		</div>
		<div class="row">
			<div
						...
						<tbody>
							<c:forEach items="${listPersons }" var="p">
								<tr>
									...
									<td>${p.name }</td>
									<td>${p.country }</td>
									<td><c:if test="${!empty p.imagen }">
											<img src="<s:url value='/myImage/imageDisplay?id=${p.id}'/>"
												style="width: 48px; height: 48px" class="img-rounded" />
										</c:if></td>
